...
class AnemoiTrainer(ABC):
    ...
    def run_id(self) -> str:
        ...
        date_str = datetime.datetime.now(tz=datetime.timezone.utc).strftime("%Y%m%d-%H%M")
        width = self.config.model.num_channels
        depth = self.config.model.processor.num_layers
        local_lr = self.config.training.lr.rate
        bs = self.config.dataloader.batch_size.training * self.config.training.accum_grad_batches \
            * self.config.hardware.num_gpus_per_node * self.config.hardware.num_nodes / self.config.hardware.num_gpus_per_model
        bs = int(bs)
        
        return f"{date_str}_{width}_{depth}_llr{local_lr}_bs{bs}"
    ...
    def _log_information(self) -> None:
        ...
        LOGGER.info("Accum grad batches: %d", self.config.training.accum_grad_batches)
        LOGGER.info("Local batch size: %d", self.config.dataloader.batch_size.training)
        LOGGER.info(
            "Effective learning rate: %.3e",
            int(total_number_of_model_instances) * self.config.training.lr.rate \
                * int(self.config.training.accum_grad_batches) \
                * int(self.config.dataloader.batch_size.training),
        )
    ...
    def train(self) -> None:
        ...
        # val_check_interval takes precedence if check_val_every_n_steps is set
        val_check_interval = getattr(self.config.diagnostics, "check_val_every_n_steps", 20) # default to 20 steps
        check_val_every_n_epoch = None if val_check_interval else getattr(
