...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        # add layerNorm to the q and k tensors 
    ...
    def forward(
        ...
        # Apply layernorm to q and k, we need to flatten the batch dims to apply layernorm which expects [N, C]
        batch_size, num_heads, seq_len, head_dim = q.shape
        q_flat = q.reshape(-1, head_dim)
        k_flat = k.reshape(-1, head_dim)

        # Apply the LayerNorm 
        q_ln = self.q_ln(q_flat).reshape(batch_size, num_heads, seq_len, head_dim)
        k_ln = self.k_ln(k_flat).reshape(batch_size, num_heads, seq_len, head_dim)
