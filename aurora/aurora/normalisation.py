...
def load_normalization_stats(stats_path: str, variable_name_mapping=None, verbose=False) -> Tuple[Dict[str, float], Dict[str, float]]:
    locations, scales = {"<N/A>": 0}, {"<N/A>": 1}
    if stats_path.endswith('.json'):
        with open(stats_path, 'r') as file:
            stats = json.load(file)
            for k, v in stats['locations'].items():
                locations[k] = v
            for k, v in stats['scales'].items():
                scales[k] = v
    else: 
        fname_mean, fname_std = join(stats_path, 'normalize_mean.npz'), join(stats_path, 'normalize_std.npz')
        if not os.path.exists(fname_mean) or not os.path.exists(fname_std):
            raise FileNotFoundError(f"Normalization stats files {fname_mean} or {fname_std} not found.")
        if variable_name_mapping is None:
            raise ValueError(
                "variable_name_mapping must be provided for .npz files. "
                "This mapping should convert CMIP6 variable names to ERA5 names."
            )
        try:
            # Load the original data
            mean_data = np.load(fname_mean)
            std_data = np.load(fname_std)
            
            # Process each key-value pair
            for k, v in mean_data.items():
                # Split the key into base name and level (if it exists)
                base_name = k.split('_')[0]
                level_suffix = k[len(base_name):] if '_' in k else ''
                
                # Map the base name if it exists in mapping
                mapped_name = variable_name_mapping.get(base_name, base_name)
                new_key = mapped_name + level_suffix
                
                locations[new_key] = v.item()
            ...
            for k, v in std_data.items():
                ...
                scales[new_key] = v.item()
        except Exception as e:
            raise ValueError(f"Error loading normalization stats from {fname_mean} or {fname_std}: {e}")
    if verbose:
        print(f'Normalisation stats from json {stats_path} are loaded.')
