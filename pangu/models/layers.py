...
class PatchEmbedding_pretrain(nn.Module):
  def __init__(self, patch_size, dim, device):
    ...
    self.conv = nn.Conv1d(in_channels=192, out_channels=dim, kernel_size=1, stride=1, device=device)
    self.conv_surface = nn.Conv1d(in_channels=112, out_channels=dim, kernel_size=1, stride=1, device=device)
  ...
  def check_image_size_2d(self, x):
    # print("x.size()", x.size()) # torch.Size([1, 71, 721, 1440])
    _, _, h, w = x.size()
  ...
  def forward(self, input, input_surface, maps, const_h):
    ...
    # norm skipped, done in the data loader
    ...
    input_surface = input_surface.reshape(input_surface.shape[0],
                                          ...
                                          -1)
    # print("input_surface 2", input_surface.shape) # TODO: why 1184?
    input_surface = self.conv_surface(input_surface)  # deprecated (1,192,65160)
...
class EarthSpecificLayer(nn.Module):
  def __init__(self, depth, dim, layer, drop_path_ratio_list, heads, use_checkpoint, device):
    ...
    for i_layer in range(depth):
        block_list['EarthSpecificBlock{}'.format(i_layer)] = EarthSpecificBlock(dim, layer, drop_path_ratio_list[i_layer], heads, device=self.device)
...
class EarthSpecificBlock(nn.Module):
  def __init__(self, dim, layer, drop_path_ratio, heads, device):
    ...
    self.layer = layer
    self.attention = EarthAttention3D(dim, layer, heads, 0, self.window_size, device=self.device)
    self.padding_front, self.padding_back = 0, 5
    # self.pad3D = nn.ConstantPad3d((0, 0, 0, 0, self.padding_front,  self.padding_back), 0)
    if self.layer in [0,3]:
      input_shape = [8,186]
    elif self.layer in [1,2]:
      input_shape = [8,96]
  ...
  def forward(self, x, Z, H, W, roll):
    ...
    # Z=8 come from 7 patched upper level variables and 1 patched surface variable
...
class EarthAttention3D(nn.Module):
  def __init__(self, dim, layer, heads, dropout_rate, window_size, device):
