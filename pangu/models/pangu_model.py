...
class PanguModel(nn.Module):
  def __init__(self, depths = [2,6,6,2], num_heads = [6, 12, 12, 6], dims = [192, 384, 384, 192], patch_size = (2, 4, 4), device=None):
    ...
    self._input_layer = PatchEmbedding_pretrain(patch_size, dims[0], device=self.device)
  ...
  def forward(self, input, input_surface, maps,const_h):
    ...
    x = self._input_layer(input, input_surface, maps, const_h) #([1, 521280, 192]) [B, spatial, C]
