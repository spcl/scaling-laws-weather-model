...
if __name__ == "__main__":
    ...
    parser.add_argument("--yaml_config", default="./config/sfno_scaling.yaml", type=str)
    ...
    parser.add_argument("--validation_interval", default=40, type=int, help="How often to run validation (in epochs)")
    ...
    parser.add_argument("--no_load_optimizer", default=False, action="store_true")
    parser.add_argument("--no_load_scheduler", default=False, action="store_true")
    parser.add_argument("--no_load_counters", default=False, action="store_true")
    parser.add_argument("--no_load_sampler", default=False, action="store_true")
    parser.add_argument("--no_load_dataloader", default=False, action="store_true")
    # checkpoint format
    ...
    params["load_optimizer"] = not args.no_load_optimizer
    params["load_scheduler"] = not args.no_load_scheduler
    params["load_counters"] = not args.no_load_counters
    params["load_sampler"] = not args.no_load_sampler
    params["load_dataloader"] = not args.no_load_dataloader
    ...
    for mp_rank in range(comm.get_size("model")):
        ...
        # Build glob pattern
        base, ext = os.path.splitext(checkpoint_fname)
        pattern = f"{base}_step*{ext}"
        ...
        if len(matches) > 0:
            try:
                ...
                logging.warning(f"Error loading checkpoint {matches[0]}: {e}")
                resuming = False
                break
        else:
            resuming = False
    ...
    if world_rank == 0 and not resuming and args.resuming:
        logging.warning("No checkpoint available found, starting from scratch, ignoring --resuming flag")

    params["resuming"] = resuming and args.resuming
    ...
    params["validation_interval"] = args.validation_interval
