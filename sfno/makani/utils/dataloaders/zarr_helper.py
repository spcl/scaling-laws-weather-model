...
def dataset_to_stacked(
  ...
  """
  # 6 + 5*13 = 71 channels, missing 100u, 100v
  new_key = ['10m_u_component_of_wind', '10m_v_component_of_wind', 
...
def get_data_loader(params: Params, files_pattern: str, train: bool):
    ...
    if train:
        ...
        dataloader = DataLoader(
            dataset,
            # Subset(dataset,range(0, len(dataset), len(dataset)//4))[:4], # 16 is the number of validation steps
            batch_size=int(params.batch_size),
            num_workers=params.num_data_workers,
            shuffle=False,
            sampler=sampler,
...
def _xarray_to_dataset(params: Params, ds: xarray.Dataset, train: bool):
    ...
    print("dt (first 10):", ds.time.dt.hour.values[:10])
