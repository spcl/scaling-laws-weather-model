    ...
    def __init__(self, params):
        ...
        if "weighted" in loss_type:
            if params.channel_weights == "auto":
                # sum of weights=6.5 for 71 channels, graphcast sum of weights=7.4, aurora=6.3
                # then normalize to 1
                channel_weights = torch.ones(params.N_out_channels, dtype=torch.float32)
                for c, chn in enumerate(params.channel_names):
                    if chn in ["10u", "10v", "tp", "sp", "msl", "tcwv"]:
                        channel_weights[c] = 0.1
                    elif chn in ["t2m"]:
                        channel_weights[c] = 1.0
                    elif chn[0] in ["z", "u", "v", "t", "r", "q"]:
                        pressure_level = float(chn[1:])
                        channel_weights[c] = pressure_level / 6025 # sum of levels is 6025
                    else:
                        print(f"Channel: {chn} not assigned a weight")
        ...
        sum_of_weights = torch.sum(channel_weights)
        print(f"Sum of weights: {sum_of_weights}")
        channel_weights = channel_weights / sum_of_weights
