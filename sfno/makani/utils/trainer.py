...
# Add safe globals for checkpoint loading with ruamel.yaml objects
try:
    from ruamel.yaml.scalarfloat import ScalarFloat
    torch.serialization.add_safe_globals([ScalarFloat])
except ImportError:
    # If ruamel.yaml is not available, we'll handle it gracefully
    pass
...
class Trainer:
    ...
    def _get_time_stats(self):
        ...
        if self.params.enable_synthetic_data:
            ...
            # TODO: calculate climatology from the data, use zero for now
            clim = torch.zeros([self.params.N_out_channels, self.params.img_crop_shape_x, self.params.img_crop_shape_y], dtype=torch.float32, device=self.device)
    ...
    def __init__(self, params, world_rank, job_type="train"):
        ...
        self.valid_interval = params.validation_interval
        ...
        # count parameters
        if params.log_to_screen:
            self.logger.info(f"Number of trainable model parameters: {count_parameters(self.model, self.device)}")
        ...
        if params.finetune and not params.resuming:
            ...
            self.restore_checkpoint(
                ...
                load_dataloader=params.load_dataloader,
            )
        ...
        self.has_complex = any(p.is_complex() for p in self.model.parameters())
    ...
    def train(self):
        ...
        for epoch in range(self.startEpoch, self.params.max_epochs):
            ...
            if (self.data_parallel_rank == 0) and (self.params.save_checkpoint != "none"):
                ...
                val_loss = valid_logs["base"].get("validation loss", loss.item())
                # Format val_loss to 6 decimal places, iters as integer
                val_loss_str = f"{val_loss:.6f}"
                step_str = f"{self.iters}"
                # Insert val_loss and step into checkpoint filenames
                checkpoint_path = self.params.checkpoint_path.replace('.tar', f'_valloss{val_loss_str}_step{step_str}.tar')
                best_checkpoint_path = self.params.best_checkpoint_path.format(mp_rank=comm.get_rank("model")).replace('.tar', f'_valloss{val_loss_str}_step{step_str}.tar')
                self.save_checkpoint(checkpoint_path, checkpoint_mode=self.params["save_checkpoint"])
                best_checkpoint_saved = os.path.isfile(best_checkpoint_path)
                if ((not self.params.skip_validation) and ((not best_checkpoint_saved) or (val_loss <= self.best_valid_loss))):
                    self.save_checkpoint(best_checkpoint_path, checkpoint_mode=self.params["save_checkpoint"])
    ...
    def train_scaling(self):
        ...
        if self.params.log_to_screen:
            ...
            self.logger.info(f"Scaffolding memory high watermark: {all_mem_gb} GB ({max_mem_gb} GB for pytorch)")
            # announce training start
            self.logger.info("Starting Training Scaling Loop...")
        ...
        # start timer
        epoch_start = time.time()
        train_time, train_data_gb, train_logs, valid_time, valid_logs = self.train_with_validation()
        ...
        if (self.data_parallel_rank == 0) and (self.params.save_checkpoint != "none"):
            ...
            if val_loss <= self.best_valid_loss:
                self.best_valid_loss = val_loss
                os.system(f"cp {checkpoint_path} {best_checkpoint_path}")
        ...
        timing_logs = {
            ...
            "training time [s]": train_time,
            "validation time [s]": valid_time,
            "training step time [ms]": (train_time / train_logs["train_steps"]) * 10**3 if train_steps > 0 else 0,
            "minimal IO rate [GB/s]": train_data_gb / train_time if train_time > 0 else 0,
        }
    ...
    def train_one_epoch(self):
        ...
        for data in tqdm(self.train_dataloader, desc="Training progress  ", disable=not self.params.log_to_screen):
            ...
            if self.graph is not None:
                ...
                # Check if we have complex numbers in the model
                if self.has_complex:
                    # Disable AMP for complex numbers
                    with torch.cuda.amp.autocast(enabled=False):
                        pred = self.model_train(inp)
                        loss = self.loss_obj(pred, tar, inp)
                    loss.backward()
                else:
                    # Use AMP for real numbers
    ...
    def train_with_validation(self):
        ...
        print(f"tqdm only shows current training step, no previous steps")
        ...
        for epoch in range(self.startEpoch, self.params.max_epochs):
            ...
            for data in tqdm(self.train_dataloader, desc="Training progress  ", disable=not self.params.log_to_screen):
                ...
                # # --- Update learning rate every training step (except ReduceLROnPlateau) ---
                # if self.scheduler is not None and self.params.scheduler != "ReduceLROnPlateau":
                #     self.scheduler.step()
                # # --- End update ---
                ...
                if self.iters % self.valid_interval == 0:
                    valid_time, valid_logs = self.validate_step()
                    self._set_train()

                    # # --- Moved logic: wandb, checkpoint, logging ---
                    # # Learning rate scheduler for ReduceLROnPlateau (only after validation)
                    # if self.params.scheduler == "ReduceLROnPlateau":
                    #     self.scheduler.step(valid_logs["base"].get("validation loss", loss.item()))

                    # wandb logging
                    if self.params.log_to_wandb:
                        for pg in self.optimizer.param_groups:
                            lr = pg["lr"]
                        wandb.log({"learning rate": lr}, step=self.iters)
                    ...
                    if (self.data_parallel_rank == 0) and (self.params.save_checkpoint != "none"):
                        ...
                        # Format val_loss to 6 decimal places, iters as integer
                        val_loss_str = f"{val_loss:.6f}"
                        t2m_loss_str = f"{valid_logs['metrics']['validation t2m']:.6f}"
                        u10_loss_str = f"{valid_logs['metrics']['validation 10u']:.6f}"
                        z500_loss_str = f"{valid_logs['metrics']['validation z500']:.6f}"
                        step_str = f"{self.iters}"
                        # Insert val_loss and step into checkpoint filenames
                        checkpoint_path = self.params.checkpoint_path.replace('.tar', f'_step{step_str}_valloss{val_loss_str}_t2m{t2m_loss_str}_u10{u10_loss_str}_z500{z500_loss_str}.tar')
                        best_checkpoint_path = self.params.best_checkpoint_path.format(mp_rank=comm.get_rank("model"))
                        self.save_checkpoint(checkpoint_path, checkpoint_mode=self.params["save_checkpoint"])
                        best_checkpoint_saved = os.path.isfile(best_checkpoint_path)
                        if ((not self.params.skip_validation) and ((not best_checkpoint_saved) or (val_loss <= self.best_valid_loss))):
                    ...
                    timing_logs = {
                        ...
                        "training time [s]": train_time,
                        "validation time [s]": valid_time,
                        "training step time [ms]": (train_time / train_steps) * 10**3 if train_steps > 0 else 0,
                        "minimal IO rate [GB/s]": total_data_gb / train_time if train_time > 0 else 0,
                    }
                    logs = {"loss": loss}
                    if dist.is_initialized():
                        for key in sorted(logs.keys()):
                    ...
                    if self.data_parallel_rank == 0:
                        logs["train_steps"] = train_steps
                        logs["global_step"] = self.iters # use iters instead of global_step, only one epoch
                        self.log_step(logs, valid_logs, timing_logs)
                    # --- End moved logic ---
                else:
                    valid_time = 0
                    valid_logs = {"base": {}, "metrics": {}}
        ...
        return train_time, total_data_gb, logs, valid_time, valid_logs
    

    def validate_step_one_gpu(self):
        # set to eval
        self._set_eval()
        ...
        with torch.inference_mode():
            with torch.no_grad():
                ...
                for data in tqdm(self.valid_dataloader, desc="Validation progress", disable=not self.params.log_to_screen):
                    ...
                    inp, tar = self.preprocessor.cache_unpredicted_features(*gdata)
                    inp = self.preprocessor.flatten_history(inp)
                    tar = self.preprocessor.flatten_history(tar) # the same as training

                    # FW pass
                    if self.has_complex:
                        with torch.cuda.amp.autocast(enabled=False):
                            pred = self.model_eval(inp)
                            loss = self.loss_obj(pred, tar, inp)
                    else:
                        with torch.cuda.amp.autocast(enabled=self.amp_enabled, dtype=self.amp_dtype):
                            pred = self.model_eval(inp)
                    ...
                    self.metrics.update(pred, tar, loss, 0) # idt=0
    ...
    def log_step(self, train_logs, valid_logs, timing_logs):
        # separator
        separator = "".join(["-" for _ in range(50)])
        print_prefix = "    "
        ...
        if self.params.log_to_screen:
            self.logger.info("Training steps: {}".format(train_logs["global_step"]))
        ...
        with torch.no_grad():
            ...
            if checkpoint_mode == "legacy":
                ...
                store_dict = {
                    ...
                    "dataloader_sampler_state_dict": self.train_dataloader.sampler.state_dict(),
                }
                ...
                if self.train_dataloader is not None:
                    store_dict["dataloader_state_dict"] = self.train_dataloader.state_dict()
                    store_dict["dataloader_sampler_state_dict"] = self.train_dataloader.sampler.state_dict()
    ...
    def restore_checkpoint(self, checkpoint_path, checkpoint_mode="flexible",
        ...
        if ("_valloss" not in checkpoint_path) or ("_step" not in checkpoint_path):
            # Insert mp_rank if needed
            checkpoint_path = checkpoint_path.format(mp_rank=mp_rank)
            # Build glob pattern
            base, ext = os.path.splitext(checkpoint_path)
            pattern = f"{base}_step*_valloss*{ext}"
            ...
            if matches:
                ...
                if self.params.log_to_screen:
                    self.logger.warning(f"No checkpoint found matching pattern: {pattern}, using: {checkpoint_path}")
                return
        else:
            checkpoint_path = checkpoint_path.format(mp_rank=mp_rank)

        if self.params.log_to_screen:
            self.logger.info("Loading checkpoint %s in %s mode" % (checkpoint_path, checkpoint_mode))
        ...
        if checkpoint_mode == "legacy":
            ...
            if "dataloader_state_dict" in checkpoint and load_dataloader:
                self.train_dataloader.load_state_dict(checkpoint["dataloader_state_dict"])
                print("Loaded dataloader state dict")
                
            if "dataloader_sampler_state_dict" in checkpoint and load_dataloader:
                self.train_dataloader.sampler.load_state_dict(checkpoint["dataloader_sampler_state_dict"])
            ...
            if "scheduler_state_dict" in checkpoint and load_scheduler and (self.scheduler is not None):
                print('load scheduler')
                self.scheduler.load_state_dict(checkpoint["scheduler_state_dict"])

            if "iters" in checkpoint and load_counters:
                self.iters = checkpoint["iters"]
                print('load counters, iter={}'.format(self.iters))
